...
// 辅助函数：在编辑器中渲染带高亮的文本
const PARAGRAPH_IMAGE_CONFIG: FieldConfig = {
  key: 'image',
  label: '段落图片',
  type: 'image',
      ...
      {segments.map((segment, index) => {
        ...
        if (vocabEntry) {
          ...
          return (
            ...
            >
              {segment.content || segment.word}
            </span>
          );
        }
        return <span key={index} className="text-luxury-text pointer-events-none" style={{ opacity: 1 }}>{segment.content || segment.word}</span>;
      })}
    </div>
  );
};
...
export const EditorMode: React.FC<EditorModeProps> = ({ initialParagraphId, initialEditVocabId }) => {
  ...
  const [projectAuthor, setProjectAuthor] = useState(currentProject?.author || '');
  ...
  useEffect(() => {
    ...
    if (currentProject?.author !== undefined) {
      setProjectAuthor(currentProject.author);
    }
  }, [currentProject?.id]);

  const handleTitleBlur = () => {
    if (projectTitle !== currentProject?.title) {
      saveProject(projectTitle, currentProject?.id);
    }
  };

  const handleAuthorBlur = () => {
    if (projectAuthor !== currentProject?.author) {
      saveProject(projectTitle, currentProject?.id, { author: projectAuthor });
    }
  };
  ...
  const [coverImageHash, setCoverImageHash] = useState<string | null>(null);

  useEffect(() => {
    if (currentProject?.coverImageData instanceof Blob) {
      const currentHash = `${currentProject.coverImageData.size}_${currentProject.coverImageData.type}`;
      if (coverImageHash !== currentHash) {
        if (coverImageUrl && coverImageUrl.startsWith('blob:')) {
          URL.revokeObjectURL(coverImageUrl);
        }
        const url = URL.createObjectURL(currentProject.coverImageData);
        setCoverImageUrl(url);
        setCoverImageHash(currentHash);
      }
  ...
  useEffect(() => {
    ...
    // 为了检测 Blob (imageData) 变化，我们需要包含其特征
    const currentContent = JSON.stringify({
      ...
      // imageData 无法直接序列化，通过记录其关键属性来辅助检测变化
      _imageHash: [
        ...(editingVocab.imagesData || (editingVocab.imageData ? [editingVocab.imageData] : [])).map(d => `${d.size}_${d.type}`),
        ...(editingVocab.images || (editingVocab.image ? [editingVocab.image] : []))
      ].join('|')
    });
  ...
  const handleEditVocab = (vocab: Vocabulary) => {
    ...
    setLastSavedVocab(JSON.stringify({
      ...
      _imageHash: [
        ...(data.imagesData || (data.imageData ? [data.imageData] : [])).map(d => `${d.size}_${d.type}`),
  ...
  const matchedVocabIds = useMemo(() => {
    ...
    for (let i = 0; i < rawSegments.length; i++) {
      ...
      if (isSpace) {
        segments.push({ type: 'space', content: current, index: i });
      } else if (isPunct) {
        segments.push({ type: 'punct', content: current, index: i });
      } else {
        segments.push({ type: 'word', content: current, index: i });
      }
    }
                    ...
                    <label className="text-xxs font-bold tracking-widest cursor-pointer hover:text-luxury-gold">
                      ...
                      <input 
                        ...
                        onChange={e => {
                          ...
                          if (file && currentProject?.id) saveProject(projectTitle, currentProject.id, { author: projectAuthor, coverImageData: file, coverImage: '' });
                        }}
                      />
                    </label>
                    <button
                      className="text-xxs font-bold tracking-widest hover:text-red-800"
                      onClick={() => currentProject?.id && saveProject(projectTitle, currentProject.id, { author: projectAuthor, coverImage: '', coverImageData: undefined })}
                    >
                      REMOVE
                    </button>
                  </div>
                ...
                <div className="flex-1 space-y-4 pt-2">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <input
                      ...
                      onChange={e => currentProject?.id && saveProject(projectTitle, currentProject.id, { author: projectAuthor, coverImage: e.target.value, coverImageData: undefined })}
                ...
                <div className="flex-1 space-y-8">
                  ...
                  <div className="flex items-center gap-12 pt-4 border-t border-luxury-text/5">
                    <div className="flex-1">
                      <FieldInput
                        config={PARAGRAPH_IMAGE_CONFIG}
                        value={para.image || ''}
                        word={para}
                        onChange={(value, imageData, images, imagesData) => {
            ...
            <div className="space-y-12">
              ...
              <VocabularyForm
                ...
                onChange={(field, value) => {
                  if (field === 'image_combined') {
                    setEditingVocab(prev => ({ 
                      ...prev, 
                      image: value.image, 
                      imageData: value.imageData,
                      images: value.images,
                      imagesData: value.imagesData
                    }));
                  } else {
                    setEditingVocab(prev => ({ ...prev, [field]: value }));
                  }
                }}
